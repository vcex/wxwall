  ...
  //file_put_contents('s.txt',$GLOBALS['HTTP_RAW_POST_DATA']);
     ...
     * */
     protected function findcj($check){
     		if($check[9]==1){
    				$reply = $this->responseText('恭喜您~查询到您已中奖，您的中奖兑换码是【'.$check[7].'】');
		        }else{
    ...
    protected function resetflag($weixin_name,$from) {
			...
			$sql_flag="replace `weixin_flag` (`openid`,`flag`,`vote`,`fakeid`,`verify`,`fromtype`) VALUES ('$from','1','$vote','$fakeid','{$xieverify}','$fromtype')";
...
class version{
    ...
    public function hasplugs(){
		...
		if(file_exists("../paipaile/index.php"))
		 {
			 array_push($plugs['plugs'],'ppl_plug');
		 }
		 return $plugs;
