				  ...
				  if($type == 'pic'){
					  $usernickname = new NewsResponseItem("亲爱的【{$q['nickname']}】","这是图片消息",$content,$content);
					   ...
					   if ($xuanzezu[9] == 0) {
										...
										$maxid = $row['id'];
										   doshenhe($maxid);
									$isauto = new NewsResponseItem("您的消息已成功传递，请多关注现场大屏幕内容！");
									}else{
									$isauto = new NewsResponseItem("您的消息已成功传递，等待管理员审核后即可上墙，请多关注现场大屏幕内容！");
									}
						$reply = array(
